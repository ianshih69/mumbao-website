  ...
  return (
    <div tabIndex={0} aria-label={ariaLabel} className="block outline-none focus:outline-none focus-visible:ring-0">
      ...
      >
        ...
        {/* Overlay (pointer events only on inner button) */}
        <div className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-within:opacity-100 pointer-events-none">
          {href ? (
            <a
              href={href}
              aria-label={ariaLabel}
              className="pointer-events-auto border border-white text-white text-base md:text-lg font-normal px-6 py-3 tracking-wide"
            >
              詳細內容 ＋
            </a>
          ) : (
            <div className="pointer-events-auto border border-white text-white text-base md:text-lg font-normal px-6 py-3 tracking-wide">
              詳細內容 ＋
            </div>
          )}
        </div>
